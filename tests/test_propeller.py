...
class PropTest(unittest.TestCase):
    ...
    def test_coefficients(self):
        ...
        self.assertTrue(
            np.isclose(self.P.Cd, 0.019334, rtol=0.01, atol=0.001), msg=self.P.Cd
        )

    def test_forces(self):
        T, M = self.P.forces(600)
        self.assertTrue(
            np.isclose(T, 0.04399 * 1.225 * 100 * 0.5**4, rtol=0.01, atol=0.001),
            msg=T,
        )
        self.assertTrue(
            np.isclose(M, 0.003578 * 1.225 * 100 * 0.5**5, rtol=0.01, atol=0.001),
            msg=M,
        )
    ...
    def test_req_rpm(self):
        ...
        self.assertTrue(
            np.isclose(
                N, (1 / 0.04399 / 1.225 / (0.5**4)) ** 0.5 * 60, rtol=0.01, atol=0.001
            ),
            msg=N,
        )
